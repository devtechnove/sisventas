...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item"><a href="{{ route('purchase-returns.index') }}">Retorno de compras</a></li>
                        ...
                        <form id="purchase-return-form" action="{{ route('purchase-returns.update', $purchase_return) }}" method="POST">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="status" required>
                                            <option {{ $purchase_return->status == 'Pendiente' ? 'selected' : '' }} value="Pendiente">Pendiente</option>
                                            <option {{ $purchase_return->status == 'Enviado' ? 'selected' : '' }} value="Enviado">Enviado</option>
                                            <option {{ $purchase_return->status == 'Completado' ? 'selected' : '' }} value="Completado">Completado</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                            ...
                            <div class="mt-3">
                                <button type="submit" class="btn btn-primary">
                                   Guardar modificaciónn <i class="bi bi-save"></i>
                                </button>
                            </div>
                        </form>
                    </div>
