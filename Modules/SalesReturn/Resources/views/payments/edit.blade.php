...
@section('title', 'Edición de pagos')

@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
        <li class="breadcrumb-item"><a href="{{ route('sale-returns.index') }}">Devoluciones</a></li>
        <li class="breadcrumb-item"><a href="{{ route('sale-returns.show', $sale_return) }}">{{ $sale_return->reference }}</a></li>
        <li class="breadcrumb-item active">Modificación de pagos</li>
    </ol>
@endsection
                                    ...
                                    <div class="from-group">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="payment_method" id="payment_method" required>
                                                <option {{ $saleReturnPayment->payment_method == 'Efectivo' ? 'selected' : '' }} value="Efectivo">Efectivo</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Tarjeta de crédito' ? 'selected' : '' }} value="Tarjeta de crédito">Tarjeta de crédito</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Transferencia bancaria' ? 'selected' : '' }} value="Transferencia bancaria">Transferencia bancaria</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Cheque' ? 'selected' : '' }} value="Cheque">Cheque</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Otros' ? 'selected' : '' }} value="Otros">Otros</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
